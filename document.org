...
*** DSL externes
    ...
    Si le langage =SQL= (voir exemple [[External-DSL-Example-with-SQL]]) limité l'utilisateur dans le cadre de la manipulation de données provenant de bases de données relationnelles, il ne faut pas voir cette limitation comme un inconvénient. Elle encourage a être plus déclaratif tout en empéchant l'écriture de code « compliqué ». De plus, si l'utilisateur n'a pas accès à toutes le fonctionnalités du système, le DSL apporte une sécurité contre les maladresses. Si cela est aussi vrai pour le DSL interne, il l'est particulièrement pour le DSL externe qui restriend l'utilisateur dans un context..
